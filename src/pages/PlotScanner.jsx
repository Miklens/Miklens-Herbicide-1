...
import {
  Camera, Sprout, Image, FileText, X, ScanLine, CheckCircle2,
  AlertTriangle, Repeat2, FlipHorizontal2, Loader2, QrCode,
  ChevronRight, Clock, MapPin, FlaskConical, User
} from 'lucide-react';
...
function resolveTrialFromQr(parsed, trials) {
  ...
  if (parsed.type === 'json') {
    ...
    return trials.find(t =>
      (d.ID && t.ID === d.ID) ||
      (d.id && t.ID === d.id) ||
      (d.FormulationName && t.FormulationName === d.FormulationName &&
       d.Date && t.Date === d.Date)
    ) || null;
  }
  // plain id
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-sm overflow-hidden flex flex-col animate-in fade-in zoom-in duration-200">
        ...
        <div className="px-5 py-4 border-b bg-emerald-50">
          ...
          <p className="text-xs text-emerald-600 mt-0.5">
            {trial ? 'Select an action for this plot' : 'No matching trial — raw data shown'}
          </p>
        </div>
        ...
        {trial ? (
          <div className="px-5 py-3 bg-white border-b text-sm space-y-1.5">
            ...
            <div className="flex gap-2 flex-wrap text-xs pt-0.5">
              ...
              <span className={`px-2 py-0.5 rounded-full ${trial.IsCompleted ? 'bg-slate-100 text-slate-500' : 'bg-amber-100 text-amber-700'}`}>
        ...
        <div className="p-4 grid grid-cols-1 gap-3">
          {actions.map(a => (
            ...
            >
              <div className={`${a.iconBg} text-white p-2.5 rounded-full shadow-md group-hover:scale-110 transition-transform shrink-0`}>
                {a.icon}
              </div>
...
function CameraModal({ mode, trialId, onClose, onCapture }) {
  ...
  return (
    <div className="fixed inset-0 bg-black z-[10001] flex flex-col">
      <button onClick={onClose} className="absolute top-4 right-4 w-10 h-10 rounded-full bg-black/50 flex items-center justify-center z-10 text-white">
        <X className="w-6 h-6" />
      </button>
      <div className="absolute top-4 left-4 z-10">
        <span className={`px-3 py-1 rounded-full text-xs font-bold text-white ${mode === 'weed' ? 'bg-emerald-500' : 'bg-blue-500'}`}>
      ...
      {error ? (
        ...
        <>
          <video ref={videoRef} className="flex-1 w-full object-cover" playsInline />
...
function ScanHistoryItem({ item, onRescan }) {
  return (
    <div className="flex items-start gap-3 p-3 bg-white rounded-xl border border-slate-100 shadow-sm">
      <div className={`mt-0.5 w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${item.matched ? 'bg-emerald-100 text-emerald-600' : 'bg-amber-100 text-amber-600'}`}>
        {item.matched ? <CheckCircle2 className="w-4 h-4" /> : <AlertTriangle className="w-4 h-4" />}
      </div>
      <div className="flex-1 min-w-0">
        ...
        {item.action && (
          <span className="inline-block mt-1 text-xs bg-slate-100 text-slate-600 px-2 py-0.5 rounded-full">{item.action}</span>
        )}
      </div>
      <button onClick={() => onRescan(item)} className="text-slate-400 hover:text-emerald-600 p-1">
...
export default function PlotScanner({ onMenuClick }) {
  ...
  const handleAction = useCallback((actionId, trial) => {
    setQuickModal(null);

    if (actionId === 'details') {
      if (trial) {
        navigate('/trials');
        window.dispatchEvent(new CustomEvent('app:openTrial', { detail: { id: trial.ID } }));
      }
      return;
    }
  ...
  const handleCapture = useCallback(async (dataUrl, mode) => {
    const { trialId } = cameraModal || {};
    setCameraModal(null);
    if (!trialId) return;

    await uploadPhoto(dataUrl, 'image/jpeg', trialId, mode);
  }, [cameraModal]);
  ...
  const handleGalleryChange = useCallback(async (e) => {
    ...
    reader.onload = async (ev) => {
      await uploadPhoto(ev.target.result, file.type || 'image/jpeg', trialId, mode);
    };
    reader.readAsDataURL(file);
  }, []);

  // ── Photo upload (add to trial state + queue sync) ────────────────────────
  const uploadPhoto = useCallback(async (dataUrl, mimeType, trialId, mode) => {
    ...
    const photoEntry = {
      tempId: `scan_${Date.now()}`,
      fileData: dataUrl,
      mimeType,
      date: now.split('T')[0],
      label: `Scan capture — ${new Date().toLocaleString()}`,
    };

    try {
      const photoKey = isWeed ? 'WeedPhotosJSON' : 'PhotoURLs';
      const existing = (() => { try { return JSON.parse(trial[photoKey] || '[]'); } catch { return []; } })();
      existing.push(photoEntry);
      trial[photoKey] = JSON.stringify(existing);
      ...
      if (result?._errType) throw new Error(result.message || 'Upload failed');

      setUploadStatus({ msg: `Photo saved to ${isWeed ? 'weed' : 'trial'} successfully!`, type: 'success' });
      setHistory(prev => prev.map((h, i) => i === 0 ? { ...h, action: isWeed ? 'Weed photo added' : 'Photo added' } : h));
    } catch (err) {
      console.error('Photo upload error:', err);
      setUploadStatus({ msg: `Saved locally. Will sync when online.`, type: 'warning' });
    }
    setTimeout(() => setUploadStatus(null), 4000);
  }, [getAppState]);

  const handleRescan = useCallback((item) => {
    if (item.trialId) {
  ...
  return (
    <div className="flex-1 flex flex-col h-full overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto bg-slate-50">
        ...
        {/* Upload status toast */}
        {uploadStatus && (
          <div className={`mx-4 mt-3 px-4 py-3 rounded-xl text-sm font-medium flex items-center gap-2 shadow
            ${uploadStatus.type === 'success' ? 'bg-emerald-50 text-emerald-800 border border-emerald-200' :
              uploadStatus.type === 'warning' ? 'bg-amber-50 text-amber-800 border border-amber-200' :
              uploadStatus.type === 'error' ? 'bg-red-50 text-red-800 border border-red-200' :
              'bg-blue-50 text-blue-800 border border-blue-200'}`}
          >
            {uploadStatus.type === 'success' && <CheckCircle2 className="w-4 h-4 shrink-0" />}
            {uploadStatus.type === 'info' && <Loader2 className="w-4 h-4 shrink-0 animate-spin" />}
            {uploadStatus.type === 'warning' && <AlertTriangle className="w-4 h-4 shrink-0" />}
            {uploadStatus.msg}
          </div>
        )}
        ...
        <div className="p-4">
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 flex flex-col items-center text-center">
            ...
            <h2 className="text-xl font-bold text-slate-800 mb-1">Plot Scanner</h2>
            <p className="text-sm text-slate-500 mb-6 max-w-xs">
              Scan QR codes on field plots to instantly add photos, identify weeds, or view trial data.
            </p>
            ...
            {recentCount > 0 && (
              <p className="mt-3 text-xs text-slate-400">
                {recentCount} trial{recentCount !== 1 ? 's' : ''} scanned this session
              </p>
            )}
          </div>
        </div>
        ...
        <div className="px-4 grid grid-cols-2 gap-3 mb-4">
          {[
            { icon: <Camera className="w-5 h-5 text-blue-500" />, bg: 'bg-blue-50', title: 'Add Photo', desc: 'Capture trial images instantly after scanning' },
            { icon: <Sprout className="w-5 h-5 text-emerald-500" />, bg: 'bg-emerald-50', title: 'Weed ID', desc: 'AI-powered weed identification from photo' },
            { icon: <Image className="w-5 h-5 text-purple-500" />, bg: 'bg-purple-50', title: 'Gallery', desc: 'Upload existing photos from your device' },
            { icon: <FileText className="w-5 h-5 text-slate-500" />, bg: 'bg-slate-50', title: 'View Details', desc: 'Open full trial data and history' },
          ].map((c, i) => (
            <div key={i} className={`${c.bg} rounded-xl p-3 border border-slate-100`}>
        ...
        {history.length > 0 && (
          <div className="px-4 mb-6">
            <h3 className="text-sm font-bold text-slate-500 uppercase tracking-wide mb-3">Recent Scans</h3>
        ...
        {history.length === 0 && (
          <div className="px-4 mb-6">
            <div className="bg-white rounded-2xl border border-dashed border-slate-200 p-8 text-center">
              ...
              <p className="text-slate-400 text-xs mt-1">Tap the button above to scan your first plot</p>
            </div>
          </div>
        )}
      </div>
